...
@dataclass
class ModelMonitortingConfig:
    monitoring_bucket :str = MODEL_MONITORING_BUCKET
    model_monitoring_dir:str =  posixpath.join(CURRENT_TIME_STEMP)+"/"
    model_monitoring_path:str = posixpath.join(model_monitoring_dir,MODEL_MONITORING_FILE_NAME)
    predicted_data_path = os.path.join(from_root(),"prediction.csv")
